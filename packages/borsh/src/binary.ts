...
const writeStringBufferFnFn: () => (
	len: number,
) => (string: string, buf: Uint8Array, offset: number) => void = () => {
	if ((globalThis as any).Buffer) {
		return (length: number) => {
			if (length < 48) return utf8.write;
			return (string: string, buf: Uint8Array, offset: number) =>
				(buf as any).write(string, offset);
		};
	}
	return () => utf8.write;
};
...
export class BinaryWriter {
	...
	public static u16(value: number, writer: BinaryWriter) {
		let offset = writer.totalSize;
		writer._writes = writer._writes.next = () =>
			writeUInt16LE(value, writer._buf, offset);
		writer.totalSize += 2;
	}
	...
	public static u64(value: number | bigint, writer: BinaryWriter) {
		let offset = writer.totalSize;
		writer._writes = writer._writes.next = () =>
			writeBigUint64Le(value, writer._buf, offset);
		writer.totalSize += 8;
	}
	...
	public static u128(value: number | bigint, writer: BinaryWriter) {
		let offset = writer.totalSize;
		writer._writes = writer._writes.next = () =>
			writeBufferLEBigInt(value, 16, writer._buf, offset);
		writer.totalSize += 16;
	}
	...
	public static u256(value: number | bigint, writer: BinaryWriter) {
		let offset = writer.totalSize;
		writer._writes = writer._writes.next = () =>
			writeBufferLEBigInt(value, 32, writer._buf, offset);
		writer.totalSize += 32;
	}
	...
	public static u512(value: number | bigint, writer: BinaryWriter) {
		let offset = writer.totalSize;
		writer._writes = writer._writes.next = () =>
			writeBufferLEBigInt(value, 64, writer._buf, offset);
		writer.totalSize += 64;
	}
	...
	public static f32(value: number, writer: BinaryWriter) {
		...
		let offset = writer.totalSize;
		writer._writes = writer._writes.next = () =>
			writeFloatLE(value, writer._buf, offset);
		writer.totalSize += 4;
	}
	...
	public static f64(value: number, writer: BinaryWriter) {
		...
		let offset = writer.totalSize;
		writer._writes = writer._writes.next = () =>
			writeDoubleLE(value, writer._buf, offset);
		writer.totalSize += 8;
	}
	...
	public static uint8ArrayCustom(
		array: Uint8Array,
		writer: BinaryWriter,
		lengthWriter: (
			len: number | bigint,
	...
	public static smallNumberEncoding(
		encoding: SmallIntegerType,
	): [(value: number, buf: Uint8Array, offset: number) => void, number] {
		if (encoding === "u8") {
			return [
				(value: number, buf: Uint8Array, offset: number) =>
	...
	public static write(
		...
		value: number | bigint | string | boolean | string,
		writer: BinaryWriter,
	) => void {
		if (encoding === "u8") {
...
export class BinaryReader {
	...
	static bool(reader: BinaryReader): boolean {
		...
		if (value !== 1 && value !== 0) {
			throw new BorshError(
				"Unexpected value for boolean: " + value + ". Expecting either 1 or 0 ",
			);
		}
	...
	static bufferString(reader: BinaryReader): string {
		...
		const string = (reader._buf as Buffer).toString(
			undefined,
			reader._offset,
			end,
		);
	...
	static bufferStringCustom(
		reader: BinaryReader,
		length: (reader: BinaryReader) => number,
	): string {
	...
	public static read(
		...
		| ((reader: BinaryReader) => number)
		| ((reader: BinaryReader) => bigint)
		| ((reader: BinaryReader) => boolean)
