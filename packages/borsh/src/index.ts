...
const PROTOTYPE_DEPENDENCY_HANDLER_OFFSET =
	PROTOTYPE_DESERIALIZATION_HANDLER_OFFSET + PROTOTYPE_POLLUTION_CONTEXT_RANGE;
const PROTOTYPE_SCHEMA_OFFSET =
	PROTOTYPE_DESERIALIZATION_HANDLER_OFFSET +
...
export function serialize(
	...
	(
		...
		(obj.constructor._borsh_serialize = serializeStruct(obj.constructor, true))
	)(obj, writer);
...
function recursiveSerialize(
	...
	(
		...
		(obj.constructor._borsh_serialize_recursive = serializeStruct(
			obj.constructor,
			false,
		))
	)(obj, writer);
	...
	try {
		const handleFn = (): ((obj: any, writer: BinaryWriter) => any) => {
			if (typeof fieldType === "string") {
				...
				if (fieldType.elementType === "u8") {
					if (fieldType instanceof FixedArrayKind) {
						return options?.unchecked
							...
							: (obj, writer) => {
									if (obj.length !== fieldType.length) {
										throw new BorshError(
											`Provided array does not equal fixed array size of field: ${fieldName}. Recieved: ${obj.length}, Expected: ${fieldType.length}`,
										);
									}
						...
						if (fieldType.sizeEncoding === "u32")
							...
							const [sizeHandle, width] = BinaryWriter.smallNumberEncoding(
								fieldType.sizeEncoding,
							);
							return (obj, writer) =>
								BinaryWriter.uint8ArrayCustom(obj, writer, sizeHandle, width);
						}
					}
				} else {
					const sizeHandle =
				...
				return (obj, writer) =>
					BinaryWriter.stringCustom(obj, writer, sizeHandle, width);
			} else {
				return (obj, writer) => {
					if (
						!options?.unchecked &&
						...
						throw new BorshError(
							`Field value of field ${fieldName} is not instance of expected Class ${getSuperMostClass(fieldType)?.name}. Got: ${obj.constructor.name}`,
						);
					}
		...
		if (!options?.unchecked) {
			return (obj: any, writer: BinaryWriter) => {
				if (obj == null && fieldType instanceof OptionKind === false) {
					throw new BorshError(
						`Trying to serialize a null value to field "${fieldName}" which is not allowed since the field is not decorated with "option(...)" but "${typeof fieldType === "function" && fieldType?.name ? fieldType?.name : fieldType}". Most likely you have forgotten to assign this value before serializing`,
					);
				}
...
function serializeStruct(ctor: Function, allowCustomSerializer = true) {
		...
		if (schema) {
			...
			if (allowCustomSerializer && schema.serializer) {
				...
				handle = prev
					? (obj, writer) => {
							prev(obj, writer);
							schema.serializer(obj, writer, (obj: any) =>
		...
		if (fieldType instanceof VecKind || fieldType instanceof FixedArrayKind) {
			if (fieldType.elementType === "u8") {
				if (fieldType instanceof FixedArrayKind) {
					...
					const sizeHandle = BinaryReader.read(
						fieldType.sizeEncoding,
						fromBuffer,
					) as (reader: BinaryReader) => number;
					return (reader) =>
						BinaryReader.uint8Array(reader, sizeHandle(reader));
				}
			} else {
				let sizeHandle =
					fieldType instanceof VecKind
						? (BinaryReader.read(fieldType.sizeEncoding, fromBuffer) as (
								reader: BinaryReader,
							) => number)
						: () => fieldType.length;
...
export function deserializeStruct(
	...
	return (reader: BinaryReader, options?: DeserializeStructOptions) => {
		...
		if (
			!options?.unchecked &&
			!(options as any)?.object &&
			!checkClazzesCompatible(result.constructor, targetClazz)
		) {
			throw new BorshError(
				`Deserialization of ${targetClazz?.name || targetClazz} yielded another Class: ${result.constructor?.name} which are not compatible`,
			);
		}
		return result;
	};
...
const getCreateDeserializationHandle = (
	clazz: any,
	offset: number,
	fromBuffer: boolean,
): ((
	...
	getDeserializationHandle(clazz, offset, fromBuffer) ||
	setDeserializationHandle(
		clazz,
		offset,
		fromBuffer,
		createDeserializeStructHandle(clazz, offset, fromBuffer),
	);
const getDeserializationHandle = (
	clazz: any,
	offset: number,
	fromBuffer: boolean,
) =>
...
const setDeserializationHandle = (
	clazz: any,
	offset: number,
	fromBuffer: boolean,
	handle: (
...
const clearDeserializeStructHandle = (
	clazz: any,
	offset: number,
	fromBuffer: boolean,
) =>
	delete clazz.prototype[
		PROTOTYPE_DESERIALIZATION_HANDLER_OFFSET +
			offset +
			(fromBuffer ? MAX_PROTOTYPE_SEARCH : 0)
	];
...
const createDeserializeStructHandle = (
	...
	let endHandle = (
		...
		return Object.assign(
			(options as any)?.construct
				? new currClazz()
				: Object.create(currClazz.prototype),
			result,
		);
	};
		...
		for (const [actualClazz, dependency] of dependencies) {
			...
			let currentVariantType =
				typeof variantIndex === "object"
					? variantIndex.length
					: (typeof variantIndex as "string" | "number");
			if (!variantType) {
				variantType = currentVariantType;
			} else if (currentVariantType !== variantType) {
				throw new Error(
					`Variant extending ${currClazz.name} have different types, expecting either number, number[] (with same sizes) or string, but not a combination of them`,
				);
			}
		...
		return (result, reader, options) => {
			...
			if (variantType === "number") {
				...
				for (const dep of variantToDepndency) {
					...
					if (
						currentVariant.length === agg.length &&
						(currentVariant as number[]).every(
			...
			if (next == undefined && dependencies) {
				...
				if (dependencies.size == 1) {
					...
					const classes = [...dependencies.entries()]
						...
						.join(", ");
					throw new BorshError(
						`Failed to find class to deserialize to from ${currClazz.name} found: ${classes} but no variant matches bytes read from the buffer.`,
					);
				}
			}
			if (next != null) {
				getCreateDeserializationHandle(next, nextOffset, fromBuffer)(
					result,
					reader,
					options,
				);
 ...
 */
const checkClazzesCompatible = (
	clazzA: Constructor<any> | AbstractType<any>,
	clazzB: Constructor<any> | AbstractType<any>,
) => {
	return (
		clazzA == clazzB ||
	...
	if (index !== undefined) {
		for (let i = offset - 1; i >= 0; i--) {
			...
			if (dependencies) {
				for (const dependency of dependencies) {
					if (dependency !== ctor) {
						let otherVariant = getVariantIndex(
							getSchema(dependency, getOffset(dependency)),
						);
						if (typeof otherVariant !== typeof index) {
							throw new BorshError(
								`Variant of ${ctor.name} have different type compared to its sibling: ${dependency.name}, expecting either number, number[] (with same sizes) or string, but not a combination of them`,
							);
						} else if (
							index === otherVariant ||
							(Array.isArray(index) &&
...
export const validate = (
	clazzes: Constructor<any> | Constructor<any>[],
	allowUndefined = false,
) => {
	return validateIterator(clazzes, allowUndefined, new Set());
};

const validateIterator = (
	clazzes: AbstractType<any> | AbstractType<any>[],
...
const validateVariantAreCompatible = (
...
export const getDiscriminator = (constructor: Constructor<any>): Uint8Array => {
	...
	for (let i = 0; i < schemas.length; i++) {
		...
		if (i !== schemas.length - 1 && clazz.fields.length > 0) {
			throw new BorshError(
				"Discriminator can not be resolved for inheritance where super class contains fields, undefined behaviour",
			);
		}
