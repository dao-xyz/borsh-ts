...
test("serialize optional field", async () => {
  const schema = new Map([
    [
      Test,
      { kind: "struct", fields: [["x", { kind: "option", type: "string" }]] },
    ],
  ]);
...
test("serialize max uint", async () => {
  ...
  expect(newValue.r.toString()).toEqual(
    "340282366920938463463374607431768211455"
  );
  expect(newValue.s.toString()).toEqual(
    "115792089237316195423570985008687907853269984665640564039457584007913129639935"
  );
  expect(newValue.t.toString()).toEqual(
...
test("serialize/deserialize fixed array", () => {
  ...
  expect(buf).toEqual(
    Buffer.from([
      5, 0, 0, 0, 104, 101, 108, 108, 111, 5, 0, 0, 0, 119, 111, 114, 108, 100,
    ])
  );
...
test("errors serializing fixed array of wrong size", () => {
  ...
  expect(() => serialize(schema, value)).toThrow(
    "Expecting byte array of length 2, but got 3 bytes"
  );
});
...
test("baseEncode string test", async () => {
  const encodedValue = baseEncode(
    "244ZQ9cgj3CQ6bWBdytfrJMuMQ1jdXLFGnr4HhvtCTnM"
  );
  const expectedValue =
    "HKk9gqNj4xb4rLdJuzT5zzJbLa4vHBdYCxQT9H99csQh6nz3Hfpqn4jtWA92";
...
test("baseDecode test", async () => {
  ...
  const expectedDecodedArray = [
    3, 96, 254, 84, 10, 240, 93, 199, 52, 244, 164, 240, 6,
  ];
